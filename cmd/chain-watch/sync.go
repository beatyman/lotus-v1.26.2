...
var hazlist = map[cid.Cid]struct{}{}
...
func syncHead(ctx context.Context, api api.FullNode, st io.Writer, ts *types.TipSet, maxBatch int) {
	...
	for len(allToSync) > 0 {
		...
		log.Infof("Storing headers:%+v", toSync)
		...
		log.Infof("Storing address mapping:%+v", addresses)
		...
		log.Infof("Storing actors:%+v", actors)
		...
		log.Info("Storing miners:%+v", miners)
		...
		log.Infof("Storing messages:%+v", msgs)
		...
		log.Infof("Storing message inclusions:%+v", incls)
		...
		log.Infof("Storing parent receipts:%+v", receipts)
