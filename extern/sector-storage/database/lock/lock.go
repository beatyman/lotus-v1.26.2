...
var (
	ErrLockedBySelf  = errors.New("already locked by self")
	ErrLockedByOther = errors.New("already locked by other")
)
...
func Lock(name string) (io.Closer, error) {
	...
	lockmu.Lock()
	defer lockmu.Unlock()
	if _, ok := locked[abs]; ok {
		return nil, ErrLockedBySelf.As(abs)
	}
...
var (
	lockmu sync.Mutex
	locked = map[string]io.Closer{} // abs path -> true
)
