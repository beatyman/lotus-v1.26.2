...
ssh "$host" 'lotus net listen' | grep -v '/172' | grep -v '/ip6' > build/bootstrap/bootstrappers.pi
sed -i "s/127.0.0.1\/tcp\/1347/120.77.213.165\/tcp\/1346/g" build/bootstrap/bootstrappers.pi
...
ssh "$host" 'lotus --repo=/data/lotus/dev/.ldt0111 send $(lotus wallet default) 100'
